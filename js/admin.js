...
function tabs(){const open=name=>{document.querySelectorAll('[data-tab]').forEach(x=>x.classList.toggle('active',x.dataset.tab===name));document.querySelectorAll('[data-panel]').forEach(p=>p.classList.toggle('hidden',p.dataset.panel!==name));history.replaceState(null,'',`#${name}`)};document.querySelectorAll('[data-tab]').forEach(btn=>btn.addEventListener('click',()=>open(btn.dataset.tab)));const wanted=location.hash.slice(1);if(wanted&&document.querySelector(`[data-tab="${wanted}"]`))open(wanted)}
...
function renderDisciplines(){const rows=$('#disciplineRows');if(!disciplines.length){rows.innerHTML='<tr><td colspan="4"><div class="empty compact">Nenhuma disciplina cadastrada.</div></td></tr>';return}rows.innerHTML=disciplines.map(d=>`<tr><td><b>${esc(d.nome)}</b></td><td><span class="badge">${Number(d.peso||1).toFixed(1)}</span></td><td><span class="badge ${d.ativo?'ok':'off'}">${d.ativo?'Ativa':'Inativa'}</span></td><td class="actions"><button class="mini" data-edit-discipline="${d.id}">Editar</button><button class="mini" data-toggle-discipline="${d.id}">${d.ativo?'Desativar':'Ativar'}</button><button class="mini danger-mini" data-delete-discipline="${d.id}">Excluir</button></td></tr>`).join('')}
function renderTopics(){const rows=$('#topicRows');if(!topics.length){rows.innerHTML='<tr><td colspan="6"><div class="empty compact">Nenhum assunto cadastrado.</div></td></tr>';return}const map=Object.fromEntries(disciplines.map(d=>[d.id,d.nome]));rows.innerHTML=topics.map(t=>`<tr><td>${esc(map[t.disciplina_id]||'—')}</td><td><b>${esc(t.nome)}</b></td><td><span class="badge">${t.recorrencia||3}</span></td><td><span class="badge">${t.dificuldade||3}</span></td><td><span class="badge ${t.ativo?'ok':'off'}">${t.ativo?'Ativo':'Inativo'}</span></td><td class="actions"><button class="mini" data-edit-topic="${t.id}">Editar</button><button class="mini" data-toggle-topic="${t.id}">${t.ativo?'Desativar':'Ativar'}</button></td></tr>`).join('')}
async function refresh(){
 const [{data:d,error:de},{data:t,error:te},{count:q,error:qe}]=await Promise.all([sb.from('disciplinas').select('*').order('peso',{ascending:false}).order('nome'),sb.from('assuntos').select('*').order('nome'),sb.from('questoes').select('*',{count:'exact',head:true})]);
 if(de||te||qe){notice('Não foi possível carregar os dados. Tente novamente.','error');return}
...
function resetDiscipline(){const f=$('#disciplineForm');f.reset();f.elements.id.value='';if(f.elements.peso)f.elements.peso.value=1;$('#cancelDiscipline').classList.add('hidden')}
function resetTopic(){const f=$('#topicForm');f.reset();f.elements.id.value='';if(f.elements.recorrencia)f.elements.recorrencia.value=3;if(f.elements.dificuldade)f.elements.dificuldade.value=3;$('#cancelTopic').classList.add('hidden')}
function bindStructure(){
 $('#disciplineForm').addEventListener('submit',async e=>{e.preventDefault();const f=e.currentTarget,id=f.elements.id.value,payload={nome:f.elements.nome.value.trim(),peso:Math.max(.1,Number(f.elements.peso?.value)||1)};if(!payload.nome)return;setBusy(f,true);const r=id?await sb.from('disciplinas').update(payload).eq('id',id):await sb.from('disciplinas').insert(payload);setBusy(f,false);if(r.error)return notice(r.error.message,'error');notice(id?'Disciplina atualizada.':'Disciplina cadastrada.');resetDiscipline();await refresh()});
 $('#topicForm').addEventListener('submit',async e=>{e.preventDefault();const f=e.currentTarget,id=f.elements.id.value,payload={disciplina_id:Number(f.elements.disciplina_id.value),nome:f.elements.nome.value.trim(),recorrencia:Number(f.elements.recorrencia?.value)||3,dificuldade:Number(f.elements.dificuldade?.value)||3};if(!payload.disciplina_id||!payload.nome)return;setBusy(f,true);const r=id?await sb.from('assuntos').update(payload).eq('id',id):await sb.from('assuntos').insert(payload);setBusy(f,false);if(r.error)return notice(r.error.message,'error');notice(id?'Assunto atualizado.':'Assunto cadastrado.');resetTopic();await refresh()});
 $('#cancelDiscipline').addEventListener('click',resetDiscipline);$('#cancelTopic').addEventListener('click',resetTopic);
 $('#disciplineRows').addEventListener('click',async e=>{const edit=e.target.dataset.editDiscipline,toggle=e.target.dataset.toggleDiscipline,del=e.target.dataset.deleteDiscipline;if(edit){const d=disciplines.find(x=>x.id===Number(edit));const f=$('#disciplineForm');f.elements.id.value=d.id;f.elements.nome.value=d.nome;if(f.elements.peso)f.elements.peso.value=d.peso||1;$('#cancelDiscipline').classList.remove('hidden');f.scrollIntoView({behavior:'smooth'})}if(toggle){const d=disciplines.find(x=>x.id===Number(toggle));const {error}=await sb.from('disciplinas').update({ativo:!d.ativo}).eq('id',d.id);if(error)return notice('Não foi possível alterar a disciplina.','error');notice(`Disciplina ${d.ativo?'desativada':'ativada'}.`);await refresh()}if(del){const d=disciplines.find(x=>x.id===Number(del));if(!d)return;if(!confirm(`Excluir a disciplina "${d.nome}"? Essa ação só será concluída se não houver conteúdo vinculado a ela.`))return;const {error}=await sb.rpc('admin_delete_disciplina',{p_id:d.id});if(error){const msg=String(error.message||'');const clean=msg.includes('DISCIPLINA_COM_CONTEUDO:')?msg.split('DISCIPLINA_COM_CONTEUDO:')[1].trim():'Não foi possível excluir a disciplina porque ainda existem conteúdos vinculados.';return notice(clean,'error')}notice('Disciplina excluída.');await refresh()}});
 $('#topicRows').addEventListener('click',async e=>{const edit=e.target.dataset.editTopic,toggle=e.target.dataset.toggleTopic;if(edit){const t=topics.find(x=>x.id===Number(edit));const f=$('#topicForm');f.elements.id.value=t.id;f.elements.disciplina_id.value=t.disciplina_id;f.elements.nome.value=t.nome;if(f.elements.recorrencia)f.elements.recorrencia.value=t.recorrencia||3;if(f.elements.dificuldade)f.elements.dificuldade.value=t.dificuldade||3;$('#cancelTopic').classList.remove('hidden');f.scrollIntoView({behavior:'smooth'})}if(toggle){const t=topics.find(x=>x.id===Number(toggle));const {error}=await sb.from('assuntos').update({ativo:!t.ativo}).eq('id',t.id);if(error)return notice(error.message,'error');notice(`Assunto ${t.ativo?'desativado':'ativado'}.`);await refresh()}});
 $('#questionDiscipline').addEventListener('change',fillQuestionTopics);
}
let participants=[];
function statusLabel(s){return {pendente:'Pendente',aprovado:'Ativo',nao_aprovado:'Não aprovado',desativado:'Desativado'}[s]||s}
...
async function loadFeature(file,exportName,args=[]){
  try{
    const mod=await import(`${file}?v=4.1.0`);
...
export async function setupAdmin(ctx){
  if(ctx.profile?.role!=='admin'){
    ...
    if(area)area.innerHTML='<div class="empty">Acesso restrito.</div>';
    return;
  }
  tabs();
  try{bindStructure()}catch(e){console.error('[Admin] estrutura:',e)}
