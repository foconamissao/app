...
let ctx,allResponses=[],allReviews=[],allErrors=[],allAttempts=[];
...
function inRange(row,start,end=null){const d=new Date(row.created_at||row.reviewed_at||row.ultimo_erro||row.finalizada_em);if(start&&d<start)return false;if(end&&d>end)return false;return true}
...
function setStats(responses,reviews,prevResponses,prevReviews,attempts=[],prevAttempts=[]){
 ...
 const regularSec=responses.filter(r=>r.origem!=='simulado').reduce((sum,r)=>sum+(Number(r.tempo_segundos)||0),0);
 const simSec=(attempts||[]).reduce((sum,a)=>sum+(Number(a.tempo_segundos)||0),0);
 const totalSec=regularSec+simSec,avg=responses.length?totalSec/responses.length:0;
 ...
 $('#perfTime').textContent=humanTime(totalSec);$('#perfAvgTime').textContent=responses.length?`Média aproximada de ${humanTime(avg)} por questão`:'Sem respostas no período';
...
function render(){
 const value=$('#performancePeriod').value,source=$('#performanceSource')?.value||'all',range=rangeFor(value);
 const periodResponses=allResponses.filter(r=>inRange(r,range.start));
 const periodAttempts=allAttempts.filter(a=>inRange(a,range.start));
 const sourceFilter=r=>source==='all'||(source==='simulado'?r.origem==='simulado':r.origem!=='simulado');
 const responses=periodResponses.filter(sourceFilter);
 const attempts=source==='questoes'?[]:periodAttempts;
 const reviews=allReviews.filter(r=>inRange(r,range.start));
 let prevResponses=[],prevReviews=[],prevAttempts=[];
 if(range.prevStart){
  prevResponses=allResponses.filter(r=>inRange(r,range.prevStart,range.prevEnd)).filter(sourceFilter);
  prevReviews=allReviews.filter(r=>inRange(r,range.prevStart,range.prevEnd));
  prevAttempts=source==='questoes'?[]:allAttempts.filter(a=>inRange(a,range.prevStart,range.prevEnd));
 }else{prevResponses=null;prevReviews=null;prevAttempts=null}
 setStats(responses,reviews,prevResponses,prevReviews,attempts,prevAttempts);
 renderSourceBreakdown(periodResponses,periodAttempts);
 renderEvolution(responses,value);renderDisciplines(responses);renderDifficulty(responses);const topics=renderTopics(responses);renderInsights(topics);renderFlash(reviews);renderErrors(allErrors,range)
}
async function load(){const uid=ctx.user.id;const [{data:r,error:re},{data:f,error:fe},{data:e,error:ee},{data:a,error:ae}]=await Promise.all([
 sb.from('respostas').select('id,questao_id,acertou,tempo_segundos,origem,referencia_id,created_at,questoes(id,dificuldade,disciplinas(nome),assuntos(nome))').eq('user_id',uid).order('created_at'),
 ...
 sb.from('simulado_tentativas').select('id,simulado_id,status,finalizada_em,tempo_segundos,pontuacao,acertos').eq('user_id',uid).eq('status','finalizada').order('finalizada_em')
 ]);if(re||fe||ee||ae){console.error(re||fe||ee||ae);notice('Não foi possível carregar seus dados de desempenho. Tente novamente.');return}allResponses=r||[];allReviews=f||[];allErrors=e||[];allAttempts=a||[];render()}
export async function setupDesempenho(context){ctx=context;$('#performancePeriod').addEventListener('change',render);$('#performanceSource')?.addEventListener('change',render);await load()}
