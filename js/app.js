...
function mountAccount(profile,container){const wrap=document.createElement('div');wrap.className='account-wrap';const img=profile.avatar_path?`<img src="${avatarUrl(profile.avatar_path)}?v=${Date.now()}" alt="Foto de ${profile.nome||'participante'}">`:`<span>${initials(profile.nome)}</span>`;wrap.innerHTML=`<div class="account-avatar">${img}</div><b class="account-name">${profile.nome||'Participante'}</b><button class="account-gear" type="button" aria-label="Opções da conta" aria-expanded="false">⚙</button><div class="account-menu hidden"><a href="${profile.role==='admin'?'./admin.html#perfil':'./perfil.html'}">👤 Editar perfil</a><button type="button" class="logout-menu">🚪 Sair</button></div>`;container.appendChild(wrap);const gear=wrap.querySelector('.account-gear'),menu=wrap.querySelector('.account-menu');gear.addEventListener('click',()=>{const opening=menu.classList.contains('hidden');menu.classList.toggle('hidden');gear.setAttribute('aria-expanded',String(opening))});wrap.querySelector('.logout-menu').addEventListener('click',signOut);document.addEventListener('click',e=>{if(!wrap.contains(e.target)){menu.classList.add('hidden');gear.setAttribute('aria-expanded','false')}})}
