...
async function loadWeek(offset=0){
  ...
  const {data,error}=await sb.from('atividades').select('id,data,titulo,descricao,tipo,disciplina_id,assunto_id,meta_quantidade,meta_minutos,ordem,disciplinas(nome),assuntos(nome),cronogramas(titulo,ativo)').gte('data',startISO).lte('data',endISO).eq('ativo',true).order('data').order('ordem');
