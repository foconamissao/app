...
let ctx,disciplines=[],topics=[],objectives=[],schedules=[],activities=[];
...
function objectiveName(id){return objectives.find(o=>Number(o.id)===Number(id))?.titulo||'Objetivo não definido'}
function activeScheduleOptions(){return schedules.filter(x=>x.ativo).map(x=>`<option value="${x.id}">${esc(x.titulo)} · ${esc(objectiveName(x.objetivo_id))}</option>`).join('')}
function fillActivitySelects(){
  const of=$('#scheduleObjective');if(of){const old=of.value;of.innerHTML='<option value="">Selecione o objetivo...</option>'+objectives.filter(x=>x.ativo).map(x=>`<option value="${x.id}">${esc(x.titulo)}</option>`).join('');if([...of.options].some(o=>o.value===old))of.value=old}
...
function renderSchedules(){
  ...
  if(!schedules.length){rows.innerHTML='<tr><td colspan="6"><div class="empty compact">Nenhum cronograma cadastrado.</div></td></tr>';return}
  rows.innerHTML=schedules.map(s=>`<tr><td><b>${esc(s.titulo)}</b></td><td>${esc(objectiveName(s.objetivo_id))}</td><td>${fmtDate(s.data_inicio)}</td><td>${fmtDate(s.data_fim)}</td><td><span class="badge ${s.ativo?'ok':'off'}">${s.ativo?'Publicado':'Oculto'}</span></td><td class="actions"><button class="mini" data-edit-schedule="${s.id}">Editar</button><button class="mini" data-toggle-schedule="${s.id}">${s.ativo?'Ocultar':'Publicar'}</button><button class="mini danger-mini" data-delete-schedule="${s.id}">Excluir</button></td></tr>`).join('');
}
...
async function refresh(){
  const [dr,tr,or,sr,ar]=await Promise.all([
    sb.from('disciplinas').select('*').order('peso',{ascending:false}).order('nome'),
    sb.from('assuntos').select('*').order('nome'),
    sb.from('concursos_objetivos').select('id,titulo,ativo').order('titulo'),
  ...
  const err=dr.error||tr.error||or.error||sr.error||ar.error;if(err){notice('Não foi possível carregar o cronograma.','error');return}
  disciplines=dr.data||[];topics=tr.data||[];objectives=or.data||[];schedules=sr.data||[];activities=ar.data||[];
...
function bindSchedules(){
  $('#scheduleForm')?.addEventListener('submit',async e=>{e.preventDefault();const f=e.currentTarget,id=f.elements.id.value;const payload={titulo:f.elements.titulo.value.trim(),objetivo_id:Number(f.elements.objetivo_id.value),data_inicio:f.elements.data_inicio.value,data_fim:f.elements.data_fim.value||null,ativo:f.elements.ativo.checked,created_by:ctx.user.id};if(!payload.objetivo_id)return notice('Selecione o objetivo deste cronograma.','error');if(payload.data_fim&&payload.data_fim<payload.data_inicio)return notice('A data final não pode ser anterior à inicial.','error');setBusy(f,true);const r=id?await sb.from('cronogramas').update(payload).eq('id',id):await sb.from('cronogramas').insert(payload);setBusy(f,false);if(r.error)return notice('Não foi possível salvar o cronograma.','error');notice(id?'Cronograma atualizado.':'Cronograma criado.');resetSchedule();await refresh()});
  ...
  $('#scheduleRows')?.addEventListener('click',async e=>{const b=e.target.closest('button');if(!b)return;const id=Number(b.dataset.editSchedule||b.dataset.toggleSchedule||b.dataset.deleteSchedule);const s=schedules.find(x=>x.id===id);if(!s)return;
    if(b.dataset.editSchedule){const f=$('#scheduleForm');f.elements.id.value=s.id;f.elements.titulo.value=s.titulo;f.elements.objetivo_id.value=s.objetivo_id||'';f.elements.data_inicio.value=s.data_inicio;f.elements.data_fim.value=s.data_fim||'';f.elements.ativo.checked=s.ativo;$('#cancelSchedule').classList.remove('hidden');f.scrollIntoView({behavior:'smooth'})}
