...
async function loadResult(attemptId,message=null){
  ...
  const [{data:resp},{data:links},{data:detailed},{data:previous}]=await Promise.all([
    sb.from('simulado_respostas').select('questao_id,resposta,acertou,marcada_revisao,questoes(id,enunciado,correta,comentario,ponto_fixacao,base_legal,alternativa_a,alternativa_b,alternativa_c,alternativa_d,alternativa_e,dificuldade,disciplinas(id,nome),assuntos(id,nome))').eq('tentativa_id',attemptId),
    sb.from('simulado_questoes').select('questao_id,peso').eq('simulado_id',a.simulado_id),
    sb.from('respostas').select('questao_id,acertou,tempo_segundos,questoes(enunciado,dificuldade,disciplinas(nome),assuntos(nome))').eq('user_id',ctx.user.id).eq('origem','simulado').eq('referencia_id',attemptId),
    sb.from('simulado_tentativas').select('id,numero_tentativa,pontuacao,acertos,tempo_segundos,finalizada_em').eq('user_id',ctx.user.id).eq('simulado_id',a.simulado_id).eq('status','finalizada').order('finalizada_em',{ascending:true})
  ]);
  const weights=Object.fromEntries((links||[]).map(x=>[x.questao_id,Number(x.peso||1)]));
  const byDisc={};(resp||[]).forEach(r=>{const n=r.questoes?.disciplinas?.nome||'Sem disciplina';if(!byDisc[n])byDisc[n]={id:r.questoes?.disciplinas?.id||null,total:0,hits:0,weight:0,hitWeight:0};const x=byDisc[n],w=weights[r.questao_id]||1;x.total++;x.weight+=w;if(r.acertou){x.hits++;x.hitWeight+=w}});
  ...
  const discStats=Object.entries(byDisc).map(([n,x])=>({n,...x,pct:x.weight?Math.round(x.hitWeight/x.weight*100):0})).sort((a,b)=>b.pct-a.pct);const answered=(resp||[]).filter(r=>r.resposta).length,total=Number(a.simulados.total_questoes||links?.length||resp?.length||0),blank=Math.max(0,total-answered);$('#resultAnswered').textContent=`${answered}/${total}`;$('#resultBlank').textContent=blank;$('#resultBestDisc').textContent=discStats[0]?.n||'—';$('#resultWeakDisc').textContent=discStats.length?discStats[discStats.length-1].n:'—';const weak=discStats.filter(x=>x.total>=2&&x.pct<70).slice(-3).reverse();$('#resultNextSteps').innerHTML=weak.length?weak.map(x=>`<a class="result-step" href="./questoes.html?disciplina=${x.id||''}&quantidade=10"><span>📉</span><div><b>Reforçar ${esc(x.n)}</b><small>${x.pct}% no simulado · ${x.hits}/${x.total} acertos</small></div><strong>PRATICAR →</strong></a>`).join(''):`<a class="result-step" href="./revisao.html"><span>✅</span><div><b>Bom equilíbrio entre disciplinas</b><small>Use a revisão do dia para consolidar os pontos que restaram.</small></div><strong>REVISAR →</strong></a>`;
  const detailRows=(detailed||[]).length?(detailed||[]):(resp||[]).map(r=>({questao_id:r.questao_id,acertou:r.acertou,tempo_segundos:0,questoes:r.questoes}));const dmap=new Map();for(const r of detailRows){const d=Number(r.questoes?.dificuldade)||3,x=dmap.get(d)||{n:0,h:0};x.n++;if(r.acertou)x.h++;dmap.set(d,x)}const dlabels={1:'Muito básica',2:'Básica',3:'Intermediária',4:'Avançada',5:'Muito avançada'};$('#difficultyResult').innerHTML=[...dmap.entries()].sort((a,b)=>a[0]-b[0]).map(([d,x])=>`<div class="sim-disc-row"><span>${dlabels[d]||`Nível ${d}`}</span><b>${x.h}/${x.n}</b><strong>${x.n?Math.round(x.h/x.n*100):0}%</strong></div>`).join('')||'<div class="empty compact">Sem classificação suficiente.</div>';const prev=(previous||[]).filter(x=>Number(x.id)!==Number(a.id));const lastPrev=prev[prev.length-1];$('#attemptComparison').innerHTML=lastPrev?`<div class="attempt-compare"><div><small>Tentativa anterior</small><b>${Number(lastPrev.pontuacao||0).toFixed(1)}%</b></div><div><small>Tentativa atual</small><b>${Number(a.pontuacao||0).toFixed(1)}%</b></div><strong>${Number(a.pontuacao||0)-Number(lastPrev.pontuacao||0)>=0?'+':''}${(Number(a.pontuacao||0)-Number(lastPrev.pontuacao||0)).toFixed(1)} p.p.</strong></div>`:'<div class="empty compact">Esta é a primeira tentativa concluída deste simulado.</div>';const slow=[...detailRows].filter(x=>Number(x.tempo_segundos)>0).sort((a,b)=>Number(b.tempo_segundos)-Number(a.tempo_segundos)).slice(0,5);$('#slowQuestions').innerHTML=slow.length?slow.map((x,i)=>`<div class="slow-row"><span>#${i+1}</span><div><b>${esc(x.questoes?.assuntos?.nome||x.questoes?.disciplinas?.nome||'Questão')}</b><small>${esc((x.questoes?.enunciado||'').slice(0,120))}</small></div><strong>${fmtTime(Number(x.tempo_segundos)||0)}</strong></div>`).join(''):'<div class="empty compact">O tempo por questão não está disponível para esta tentativa.</div>';
