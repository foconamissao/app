...
function renderReview(){
 ...
 const c=queue[pos]; revealed=false;
...
async function rate(rating){
 const c=queue[pos],buttons=$('#ratingButtons'); buttons.querySelectorAll('button').forEach(b=>b.disabled=true);
 const {data,error}=await sb.rpc('registrar_revisao_flashcard',{p_flashcard_id:Number(c.id),p_avaliacao:Number(rating)});
 buttons.querySelectorAll('button').forEach(b=>b.disabled=false);
 if(error){console.error(error);notice('Não foi possível registrar a revisão. Tente novamente.','error');return}
 const row=Array.isArray(data)?data[0]:data;
 if(row)reviews.push(row); else {
   const {data:last}=await sb.from('flashcard_revisoes').select('*').eq('user_id',ctx.user.id).eq('flashcard_id',c.id).order('reviewed_at',{ascending:false}).limit(1).maybeSingle();
   if(last)reviews.push(last);
 }
 indexReviews(); sessionStats.total++; sessionStats.ratings[rating-1]++; await syncFlashMission();
...
async function startSession(){
 ...
 const ids=due.map(c=>Number(c.id));
 const {data,error}=await sb.rpc('selecionar_flashcards_inteligentes',{p_ids:ids,p_quantidade:Math.min(qty,due.length)});
 if(error){console.error(error);notice('Não foi possível preparar a revisão. Tente novamente.','error');return}
 const map=new Map(due.map(c=>[Number(c.id),c]));
 queue=(data||[]).map(x=>map.get(Number(x.flashcard_id??x.id))).filter(Boolean);
 if(!queue.length)queue=due.slice(0,Math.min(qty,due.length));
 pos=0;sessionStats={total:0,ratings:[0,0,0,0]};requeued=new Map();renderReview();
}
